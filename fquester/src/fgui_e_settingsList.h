...
namespace FGUI {
	namespace Elements {
		class SettingsList : public Element {
			bool firstSize;

			int down;
			std::vector <FGUI::Elements::StsText> texts;
		...
		public:
			...
			std::vector <FGUI::Elements::StsText>* getTexts() {
				return &texts;
			}
			double getLimitDown() {
				if ((texts.size() - int(size.y / Vars::SettingsList::heightLine)) < .0) return .0;
				else return (texts.size() - int(size.y / Vars::SettingsList::heightLine));
			}
			...
			void addText(std::wstring _text) {
				if (texts.size() < Vars::SettingsList::maxLines) {
					texts.resize(texts.size() + 1);
					texts[texts.size() - 1].setText(_text);
				}
				else {
					for (int i = 1; i < Vars::SettingsList::maxLines; i++) {
						std::swap(texts[i - 1], texts[i]);
					}
					texts[Vars::SettingsList::maxLines - 1].setText(_text);
				}
				if (!hide) mr = true;
			}
			///////////////////
				...
				if (limitRectCheck(rch, iaw)) {
					if (FGUI::CheckRect(rch, cursor.getPosition())) {
						if (p_event->type == SDL_MOUSEBUTTONDOWN && p_event->button.button == SDL_BUTTON_LEFT) {
							if (texts.size() > 0) {
								int temp = (cursor.getPosition().y - iaw.getPosition().y - position.y - Vars::Window::hatSize) / Vars::SettingsList::heightLine;
						...
						if (p_event->type == SDL_MOUSEWHEEL)
						{
							...
							{
								if (!(texts.size() <= (int)((float)size.y / Vars::SettingsList::heightLine))) {
									down += Vars::mouseWhellSpeed;
			...
			void display(FGUI::Render::RendererPack & rp) override {
				...
				if (mr) {
					rp.renderSettingsList(position, size, down, tcur, texts);
					mr = false;
				}
			}
		...
		SettingsList* CreateSettingsList(FGUI::Vector2i _position, FGUI::Vector2u _size) {
			SettingsList* bufElement = new SettingsList;
